...
import { Building2, Briefcase, CreditCard, User, Server, Copyright, Scale } from "lucide-react";
...
const sectionIcons = [Building2, Briefcase, CreditCard, User, Server, Copyright, Scale];
const sectionKeys = ["identity", "activity", "regulation", "publication", "hosting", "ip", "law"] as const;

export function LegalNoticeContent() {
  const { locale } = useLanguage();
  const t = messages[locale as keyof typeof messages]?.legalNotice || messages.fr.legalNotice;
                  ...
                  <div className="bg-dark border border-gold/10 hover:border-gold/20 transition-colors p-8 md:p-10">
                    ...
                    <div className="prose prose-invert prose-gold max-w-none">
                      {section.content.split("\n\n").map((paragraph, pIndex) => (
                        <p key={pIndex} className="text-gray-400 leading-relaxed mb-4 last:mb-0">
                          {paragraph}
                        </p>
                      ))}
                    </div>
