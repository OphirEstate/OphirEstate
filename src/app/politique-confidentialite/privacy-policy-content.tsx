...
import { Shield, Database, Target, Scale, Users, Clock, Globe, UserCheck } from "lucide-react";
...
const sectionIcons = [Shield, Database, Target, Scale, Users, Clock, Globe, UserCheck];
const sectionKeys = ["object", "data", "purposes", "legal", "recipients", "duration", "international", "rights"] as const;

export function PrivacyPolicyContent() {
  const { locale } = useLanguage();
  const t = messages[locale as keyof typeof messages]?.privacy || messages.fr.privacy;
                    ...
                    <div className="prose prose-invert prose-gold max-w-none">
                      {section.content.split("\n").map((line, lIndex) => {
                        if (line.startsWith("•")) {
                          ...
                          return (
                            <p key={lIndex} className="text-gray-400 leading-relaxed">
                              {line}
                            </p>
                          );
                        }
